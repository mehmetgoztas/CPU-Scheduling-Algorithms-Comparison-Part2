...
repeat_schedule:
	...
	if (unlikely(!c)) {
		...
		next = idle_task(this_cpu); //bekleyen processleri nexte atar
		yaslan = max; //yaslan = 420
		list_for_each(tmp,&runqueue_head){ //tum processleri gezmece
			p = list_entry(tmp, struct task_struct, run_list);
			if(can_schedule(p, this_cpu)){
				if(p->min < yaslan){
					yaslan = p->min, next = p;
				//yaslani kucuk olana esitledik
				} 
			}
		}
		if(yaslan < max){//maxtan kucuk mi diye baktik
			next->min = next->min + next->stride;
			//artis miktari kadar arttirdik
		}
		else if(yaslan == max){//yaslan maximumda 
			struct task_struct *p;

			spin_unlock_irq(&runqueue_lock);
			read_lock(&tasklist_lock);
			for_each_task(p)
				p->min = 0;//yaslani minimuma getirdik
			read_unlock(&tasklist_lock);
			spin_lock_irq(&runqueue_lock);
			goto repeat_schedule; 
		}
